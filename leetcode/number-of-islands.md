...
# Number of Islands II
 ...
 `m x n` 2D 그리드의 *사이즈*가 주어진다. 처음에 모든 그리드는 `0`, 즉
 물 상태다.

 각 단계마다 물을 땅으로 바꾸는 조작(`0 -> 1`)을 할 수 있다. 위치 배열
 `positions`가 입력으로 같이 들어오는데, `positions[i] = (r_i, c_i)`
 이고 `(r_i, c_i)` 위치의 물을 땅으로 바꾸는 조작을 한다. 그리고 이
 작업은 `i` 번째 단계에 수행되어야 한다.
...
```
 ...
 연산인데, 다시 말해 `x`와 `y`의 대표 원소를 합쳐버리는
 연산이다. 여기서는 `y`의 대표 원소를 택했다.

 근데 이렇게 나이브하게 구현하면 트리에서 Skewed Tree가 발생하는 것과
 같은 이치로, 점점 균형이 깨질 것이다. 그러면 복잡도가 `O(n)`에
 수렴한다. 좋지 않다.

 이를 위해 두 가지 최적화 방법이 있는데,
 1. Union by Rank: Rank를 기록해서, `Union` 연산을 할 때 항상 더 작은 길이의 트리를
    더 큰 길이의 트리에 합치는 방법이다.
