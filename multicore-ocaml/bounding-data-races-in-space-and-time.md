...
```
 ...
 So, providing local DRF requires us to prevent loads being reordered
 with later stores, which constrains both compiler optimisations and
 compilation to weakly-ordered hardware. 이 제약으로 인한 성능 비용이
 어느 정도인지 나중에 살펴볼 것이다.
